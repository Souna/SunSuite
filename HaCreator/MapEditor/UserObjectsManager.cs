...
namespace HaCreator.MapEditor
{
    public class UserObjectsManager
    {
        ...
        public UserObjectsManager(MultiBoard multiBoard)
        {
            ...
            {
                Program.InfoManager.ObjectSets[oS] = new SunImage(oS);
                Program.InfoManager.ObjectSets[oS].Changed = true;
            }
            SunImage osimg = Program.InfoManager.ObjectSets[oS];
            if (osimg[l0] == null)
            {
                osimg[l0] = new SunSubProperty();
            }
            SunProperty l0prop = osimg[l0];
            if (l0prop[l1] == null)
            {
                l0prop[l1] = new SunSubProperty();
            }
            l1prop = l0prop[l1];
        }
        ...
        public ObjectInfo Add(Bitmap bmp, string name)
        {
            ...
            SunCanvasProperty canvasProp = new SunCanvasProperty();
            canvasProp.PNG = new SunPngProperty();
            canvasProp.PNG.SetPNG(bmp);
            canvasProp["origin"] = new SunVectorProperty("", new SunIntProperty("X", origin.X), new SunIntProperty("Y", origin.Y));
            canvasProp["z"] = new SunIntProperty("", 0);
            prop["0"] = canvasProp;
        ...
        public void Remove(string l2)
        {
            ...
            foreach (SunProperty prop in l1prop.SunProperties)
            {
                if (prop.Name == l2)
                {
                    prop.Remove();
        ...
        public bool Dirty
        { get { return dirty; } set { dirty = value; } }
    }
}