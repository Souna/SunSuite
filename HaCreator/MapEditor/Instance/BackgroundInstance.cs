...
namespace HaCreator.MapEditor.Instance
{
    public class BackgroundInstance : BoardItem, IFlippable, ISerializable
    {
        ...
        private bool flip; //1 if the background is mirrored about the x axis, 0 otherwise
        private int _a; //alpha
        private int _cx; //tiling width
        private int _cy; //tiling height
        private int _rx; //horizonal velocity - used for either parallax or movement, depending on _type
        private int _ry; //vertical velocity - used for either parallax or movement, depending on _type
        private bool _front; //1 if the background is a foreground (rendered in front), 0 otherwise (rendered in back)
        ...
        public int BaseX
        { get { return (int)base.position.X; } set { base.position.X = value; } }

        public int BaseY
        { get { return (int)base.position.Y; } set { base.position.Y = value; } }
