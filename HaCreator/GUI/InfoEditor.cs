...
namespace HaCreator.GUI
{
    public partial class InfoEditor : EditorBase
    {
        ...
        public InfoEditor(Board board, MapInfo info, MultiBoard multiBoard)
        {
            ...
            foreach (KeyValuePair<string, SunSoundProperty> bgm in Program.InfoManager.BGMs)
            ...
            foreach (SunProperty prop in info.additionalProps)
            {
                TreeNode node = unknownProps.Nodes.Add(prop.Name);
                node.Tag = prop;
                if (prop.SunProperties != null && prop.SunProperties.Count > 0)
                    ExtractPropList(prop.SunProperties, node);
            }
        }

        private void ExtractPropList(List<SunProperty> properties, TreeNode parent)
        {
