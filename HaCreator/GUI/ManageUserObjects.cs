...
namespace HaCreator.GUI
{
    public partial class ManageUserObjects : Form
    {
        ...
        {
            List<SunProperty> oldProps = new List<SunProperty>();
            List<SunProperty> newProps = new List<SunProperty>();
            foreach (SunProperty prop in userObjs.L1Property.SunProperties)
            {
        ...
        private List<string> SearchMapWzForObj(string l2)
        {
            ...
            foreach (SunDirectory mapDir in ((SunDirectory)Program.WzManager["map"]["Map"]).SubDirectories)
            {
                foreach (SunImage mapImg in mapDir.SunImages)
                {
                    ...
                    if (!parsed)
                        mapImg.ParseImage();
                    foreach (SunProperty layer in mapImg.SunProperties)
                    {
                        if (layer.Name.Length != 1 || !char.IsDigit(layer.Name[0]))
                            continue;
                        SunProperty prop = layer["obj"];
                        if (prop == null)
                            continue;
                        foreach (SunProperty obj in prop.SunProperties)
                        {
    ...
    {
        public bool newObj;
        public SunProperty prop;

        public UserObject(SunProperty prop, bool newObj)
        {
            this.prop = prop;
            this.newObj = newObj;
        }
